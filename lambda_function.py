...
class DynamoAccessor:
    ...
    def get_data_from_dynamo(self, uuid):
        response = self.table.query(KeyConditionExpression=Key('uuid').eq(uuid))
        return response["Items"][0] if any(response["Items"]) else None

def lambda_handler(event, context):
    dynamo_backend = DynamoAccessor(DYNAMO_BD)
    db_element = dynamo_backend.get_data_from_dynamo(event['uuid'])
    return db_element